...
``` sql
DELIMITER $$
  CREATE PROCEDURE   add_return_records(IN p_return_id varchar(15),
...
AS
   SELECT * FROM members
   WHERE member_id IN (SELECT DISTINCT issued_member_id FROM  issue_status
                        WHERE issued_date >= DATE_SUB(CURRENT_DATE, INTERVAL 3000 DAY))
```
...
-- all the code
    ...
    IF v_status = 'yes' THEN
     ...
     ELSE
     SELECT CONCAT('Sorry, the book you have requested is unavailable. Book ISBN: ', p_issued_book_isbn) AS message;
     END IF;
END $$
 DELIMITER ;
```
